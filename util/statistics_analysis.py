...
def analyze_images_in_folder(folder_path, iterations=20):
    ...
    with open(stats_file_path, "w") as stats_file:
        stats_file.write("Time Point\tCore Mean\tCore Std\tShell Mean\tShell Std\n")
        ...
        for idx, filename in enumerate(sorted(os.listdir(folder_path))):
            if filename.endswith('.tif'):
                ...
                stats_file.write(f"{idx}\t{core_mean:.3f}\t{core_std:.3f}\t{shell_mean:.3f}\t{shell_std:.3f}\n")

    return core_means, core_stds, shell_means, shell_stds
...
def plot_histograms(folder_path, core, shell, time_point):
    ...
    core_hist_file = os.path.join(histogram_folder, f"core_histogram_time_{time_point}.txt")
    shell_hist_file = os.path.join(histogram_folder, f"shell_histogram_time_{time_point}.txt")
...
def main(folder_path, iterations=20):
    core_means, core_stds, shell_means, shell_stds = analyze_images_in_folder(folder_path, iterations)
    time_points = np.arange(len(core_means))

    # Plot statistics over time
    plot_statistics(time_points, core_means, core_stds, shell_means, shell_stds)
