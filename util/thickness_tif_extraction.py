...
output_dir = os.path.join(input_dir, 'thickness_output')
...
if not h5_files:
    ...
    for file_path in h5_files:
        ...
        # 출력할 TIF 파일의 경로와 이름 지정 ('_thickness'로 변경)
        output_filename = os.path.splitext(filename)[0] + '_thickness.tif'
        output_file_path = os.path.join(output_dir, output_filename)

        try:
            # HDF5 파일 열기
            with h5py.File(file_path, 'r') as h5file:
                # 'Thickness' 데이터가 있는지 확인
                if 'Thickness' in h5file:
                    thickness_array = h5file['Thickness'][:]

                    # TIFF 파일로 저장
                    tifffile.imwrite(output_file_path, thickness_array)
                    print(f"✅ 저장 완료: {output_filename}")
                else:
                    print(f"⚠️ 건너뜀: '{filename}' 내에 'Thickness' 데이터가 없습니다.")
